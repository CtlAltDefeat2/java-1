...
public class student6 {
   ...
   // private static Boolean passChecker() {
...
//    }
    private static void ex9() {
        ...
        for (int i =0; i < 10; i++) {
            myArray.add(random.nextInt(1,1000));
        }
        System.out.print(addNumbers(myArray));
        var result = checkArray(myArray);
        System.out.println(" " + result);
    }
    private static String checkArray(ArrayList<Integer> myArray) {
        var result = (addNumbers(myArray) % 2 == 0) ? "even" : "odd";
        return result;
    }
    private static void ex8() {
        ...
        var price = (addNumbers(numList));
        var price2 = shipHandFee(price);
        System.out.println(price2);
    }
    private static Float shipHandFee(int price) {
        var taxTotal = taxFee(price);
        float totalPrice = 0.0f;
        if (taxTotal < 10)
        {
            boolean underTen = true;
            totalPrice = taxTotal + 5;
            return totalPrice;
        }
        else if (taxTotal >= 10 && taxTotal <= 20)
        {
            boolean tenToTwenty = true;
            totalPrice = taxTotal + 4;
            return totalPrice;
        }
        else {totalPrice = taxTotal;
            return totalPrice;
        }
    }
    private static Float taxFee(int price) {
        float salesTax = .1f;
        float taxTotal = price * (salesTax + 1);
        return taxTotal;
    }
